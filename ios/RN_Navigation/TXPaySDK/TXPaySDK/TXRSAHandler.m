...
@interface TXRSAHandler ()

@property (nonatomic, copy) NSString *encryptedString;
@property (nonatomic, copy) NSString *decryptedString;

@end
@implementation TXRSAHandler
{
    RSA* _rsa_pub;
    RSA* _rsa_pri;
}
...
// 导入公钥字符串 并公钥加密
RCT_EXPORT_METHOD(ocImportPublicKey:(NSString *)keyString encryptContent:(NSString *)content) {
  [self importKeyWithType:KeyTypePublic andkeyString:keyString];
  self.encryptedString = [self encryptWithPublicKey:content];
}

// 导入私钥字符串 并私钥解密
RCT_EXPORT_METHOD(ocImportPrivateKey:(NSString *)keyString decryptContent:(NSString *)content) {
  [self importKeyWithType:KeyTypePrivate andkeyString:keyString];
  self.decryptedString = [self decryptWithPrivatecKey:content];
}

// 加密 结果回调
RCT_EXPORT_METHOD(ocEncryptedResult:(RCTResponseSenderBlock)callback) {
  if (!self.encryptedString) {
    self.encryptedString = @"加密结果为空";
  }
  NSArray *events = @[self.encryptedString];
  callback(@[[NSNull null], events]);
}

// 解密 结果回调
RCT_EXPORT_METHOD(ocDecryptedResult:(RCTResponseSenderBlock)callback) {
  if (!self.decryptedString) {
    self.decryptedString = @"解密结果为空";
  }
  NSArray *events = @[self.decryptedString];
  callback(@[[NSNull null], events]);
}
...
+ (instancetype)sharedInstance {
    static TXRSAHandler *handle = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        handle = [[TXRSAHandler alloc] init];
    });
    return handle;
}
